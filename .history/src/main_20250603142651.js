...
// Function to extract Bitcoin address from filename
...
function extractAddressAndIndexFromFilename(filename) {
  ...
  // Try new format first: 1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa_i0042
  const newFormatPattern = /^(1[A-HJ-NP-Z0-9]{25,34})_i(\d+)$/;
...
ipcMain.handle('process-files', async (event, filePaths, outputFolder, shouldDeleteSourceFolders = false, sourceFolders = []) => {
  ...
  try {
    ...
    // Updated pattern to handle both old and new formats
    const bitcoinAddressPattern = /^1[A-HJ-NP-Z0-9]{25,34}(_i\d+)?\.(jpg|jpeg)$/i;
    ...
    try {
      ...
      if (addressInfo) {
        ...
        if (validation.isValid) {
          ...
          if (addressInfo.format === 'old') {
            targetFileName = `${addressInfo.address}_i${String(finalIndex).padStart(4, '0')}${extension}`;
          }
          ...
          if (await fs.pathExists(newFilePath)) {
            ...
            results.push({
              ...
              action: addressInfo.format === 'old' ? 'upgraded_format' : 'copied_existing_address',
      ...
      if (needsRename) {
        ...
        const newFileName = `${bitcoinAddress}_i${String(addressIndex).padStart(4, '0')}${extension}`;
        ...
        if (await fs.pathExists(newFilePath)) {
          ...
          while (await fs.pathExists(uniqueNewFilePath) && attempts < 10) {
            ...
            const uniqueFileName = `${uniqueAddress}_i${String(uniqueIndex).padStart(4, '0')}${extension}`;
          ...
          newFileName = `${uniqueAddress}_i${String(uniqueIndex).padStart(4, '0')}${extension}`;
        }

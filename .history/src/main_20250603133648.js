...
function generateHDBitcoinAddress(index) {
  ...
  // Derive private key from master seed and index
  const privateKey = derivePrivateKey(masterSeed, index);
  
  // Create public key hash (simplified)
  ...
  for (const filePath of filePaths) {
    try {
      ...
      if (await fs.pathExists(newFilePath)) {
        // This should be extremely rare with HD addresses, but handle it
        let attempts = 0;
      ...
      // Add this file's hash to our processed set

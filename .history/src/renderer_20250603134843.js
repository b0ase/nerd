...
async function handleDrop(e) {
    ...
    // If folders were dropped, process them
    if (droppedFolders.length > 0) {
        showLoading('Scanning folders for JPEG files...');
        try {
            // Use the main process to scan folders
            const files = Array.from(e.dataTransfer.files);
            const folderPaths = files.filter(file => file.type === '').map(file => file.path);
            
            if (folderPaths.length > 0) {
                const result = await window.electronAPI.selectFolders();
                // This is a workaround since we can't directly get folder paths from drag/drop
                // We'll show a message asking users to use the folder selection button
                showError('For folder drag & drop, please use the "📂 Select Folders" button instead. Drag & drop works best for individual files.');
                return;
            }
        } catch (error) {
            showError('Error processing dropped folders. Please use the folder selection button.');
            return;
        }
    }
...
async function selectOutputFolder() {
    try {
        ...
        if (folder) {
            outputFolder = folder;
            // Make it very clear this is the OUTPUT folder
            ...
            updateWorkflowSteps();
            clearResults();
            showSuccess('Output folder selected successfully! Your renamed files will be saved here.');
        }
    } catch (error) {
...
function updateDangerZone() {
    if (elements.deleteSourceFolders.checked && sourceFolders.length > 0) {
        ...
        elements.foldersToDelete.innerHTML = `
            <strong>Folders that will be DELETED:</strong><br>
...
async function processFiles() {
    ...
    if (shouldDeleteFolders && sourceFolders.length > 0) {
        const confirmed = confirm(
            `⚠️ WARNING: This will PERMANENTLY DELETE the following folders after processing:\n\n${sourceFolders.join('\n')}\n\nThis action cannot be undone. Are you sure?`
        );
        if (!confirmed) {
            return;
        }

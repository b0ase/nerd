    ...
    try {
      ...
      if (needsRename) {
        ...
        results.push({
          ...
          action: (addressInfo.format === 'old' || addressInfo.format === 'legacy') ? 'upgraded_format' : 'copied_existing_address',
          message: addressInfo.format === 'old' ? 
            `Upgraded filename format to include derivation index (${addressIndex})` :
            addressInfo.format === 'legacy' ?
            `Upgraded filename format from _i to (i) notation (${addressIndex})` :
            `File already had valid address with index (${addressIndex})`
        });

...
const BACDSAPIServer = require('./api-server');
...
async function createWindow() {
  // Start the BACDS API server
  try {
    apiServer = new BACDSAPIServer();
    await apiServer.start();
    console.log('✅ BACDS API Server started successfully');
  } catch (error) {
    console.error('❌ Failed to start BACDS API Server:', error);
  }
...
async function saveAddressMapping(outputFolder, address, index, filename) {
  ...
  try {
    ...
    // Automatically generate public mapping
    await generatePublicAddressMapping(outputFolder);
...
ipcMain.handle('generate-new-master-key', async () => {
  masterSeed = generateMasterSeed();
  addressIndex = 0;
  syncMasterSeedWithAPI(); // Sync with API server
  return {
    masterKey: masterSeed.toString('hex'),
...
// Sync master seed between API server and main process
function syncMasterSeedWithAPI() {
  if (apiServer && masterSeed) {
    apiServer.masterSeed = masterSeed;
    apiServer.addressIndex = addressIndex;
  }
} 